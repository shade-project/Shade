...
BitcoinGUI::BitcoinGUI(QWidget *parent):
    ...
    setWindowTitle(tr("Shade") + " " + tr("Wallet"));
    qApp->setStyleSheet("QMainWindow { background:rgb(45, 45, 45); color:#fff; font-family:'Bebas Neue'; } #toolbar2 { border:none;width:28px; background:rgb(0, 155, 225); }");
#ifndef Q_OS_MAC
    ...
    actionConvertCurrency->setToolTip("Convert Shade to USD and BTC.");
    ...
    actionHowToStake->setToolTip("Shade Helpbook.");

    actionFacebook = new QAction(QIcon(":/icons/facebook"), tr("Visit our Website!"), this);
    actionTwitter = new QAction(QIcon(":/icons/twitter"), tr("Follow us on Twitter!"), this);
    actionReddit = new QAction(QIcon(":/icons/reddit"), tr("Official Github!"), this);
...
void BitcoinGUI::createActions() {
    ...
    chatAction->setToolTip(tr("Connect to Shade IRC"));
    ...
    sendCoinsAction = new QAction(QIcon(":/icons/null"), tr("&Send Shade"), this);
    sendCoinsAction->setCheckable(true);
    sendCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_2));
    toolbarsend2->addAction(sendCoinsAction);
    tabGroup->addAction(sendCoinsAction);

    receiveCoinsAction = new QAction(QIcon(":/icons/null"), tr("&Receive Shade"), this);
...
void BitcoinGUI::createToolBars() {
    ...
    toolbar->setStyleSheet("#toolbar { font-weight:600;border:none;height:100%;padding-top:20px; background: rgb(45, 45, 45); text-align: left; color: white;min-width:180px;max-width:180px;} QToolBar QToolButton:hover {background:rgba(28, 29, 33, 100); color:#0099dc;} QToolBar QToolButton:checked {background:rgba(28, 29, 33, 100);color:#0099dc;}  QToolBar QToolButton { font-weight:600;font-size:10px;font-family:'Arial Narrow';padding-left:20px;padding-right:181px;padding-top:5px;padding-bottom:5px; width:100%; color: white; text-align: left; background:transparent;text-transform:uppercase; }");
...
void BitcoinGUI::setClientModel(ClientModel *clientModel) {
    ...
    if (clientModel) {
        ...
        if (clientModel->isTestNet()) {
            ...
            if (trayIcon) {
                trayIcon->setToolTip(tr("Shade client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon() {
    ...
    trayIcon->setToolTip(tr("Shade client"));
...
void BitcoinGUI::setNumConnections(int count) {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Shade network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event) {
    if (event->mimeData()->hasUrls()) {
        ...
        if (nValidUrisFound) {
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Shade address or malformed URI parameters."));
        }
    }
...
void BitcoinGUI::openFacebook() {
    QDesktopServices::openUrl(QUrl("http://shadecoin.com"));
}
void BitcoinGUI::openTwitter() {
    QDesktopServices::openUrl(QUrl("https://twitter.com/ShadeCoin"));
}
void BitcoinGUI::openReddit() {
    QDesktopServices::openUrl(QUrl("https://github.com/shade-project/shade"));
}
